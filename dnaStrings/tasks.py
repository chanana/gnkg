...
def findProtein(self, job_name=None, dna_string=None):
    ...
    # load dictionary with mapping of description to the NC number
    ...
    result = get_results(files, dna) # actually search the fasta files
    if result:
        loc, prot, nc = get_useful_stuff_from_result(result) # regexes to parse results
        model_instance.dna_result = (
            f"Your DNA string was found in {nc_to_description[nc]} at {loc} as a part of {prot}."
        )
        model_instance.save() # save in database
    else:
...
def get_useful_stuff_from_result(result):
    ...
    # explanation of regex: find either "location=[number..number]" or
    # "location=complement(number..number)"
    ...
    # perhaps a convoluted way of removing "protein_id=" and the final "]"
    final_protein = (
...
# since the file list itself is randomized, ideally you'd get different results
# each time unless there is only one instance of the string in all the files.
